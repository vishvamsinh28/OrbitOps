...
async function getMaintenanceData(organizationId) {
  return listMaintenanceData(organizationId);
}

export default async function MaintenancePage() {
  const user = await requireUser();
  const canManage = canManageAssets(user.role);
  const { assets, requests } = await getMaintenanceData(user.organization?._id);
      ...
      <div className="grid gap-6 min-[961px]:grid-cols-[0.7fr_1fr]">
        <Panel>
          ...
          <form action={createMaintenanceAction} className="mt-5 grid gap-4">
            ...
            <TextField name="attachmentUrl" label="Photo or document URL" />
            <FileField
              name="attachmentFile"
            ...
            {requests.map((request) => (
              ...
              >
                ...
                <p className="text-sm text-[#586180]">
                  {request.technician ? `Technician ${request.technician}` : "No technician assigned"}
                  {request.resolutionNotes ? ` · ${request.resolutionNotes}` : ""}
                </p>
                {request.attachmentUrl ? (
                  <a
                    href={request.attachmentUrl}
                    className="font-mono text-xs text-[#FFB020]"
                  >
                    Attachment
                  </a>
                ) : null}
